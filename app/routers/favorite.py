...
async def add_favorite(
    store_id: int, payload: FavoriteCreate, session: AsyncSession = Depends(get_session)
) -> dict:
    ...
    stmt = (
        pg_insert(Favorite)
        .values(user_id=payload.user_id, store_id=store_id)
...
async def remove_favorite(
    store_id: int, user_id: int, session: AsyncSession = Depends(get_session)
) -> Response:
    # 없어도 에러 없이 204(멱등).
    await session.execute(
        delete(Favorite).where(Favorite.user_id == user_id, Favorite.store_id == store_id)
    )
    await session.commit()
...
async def list_favorites(
    ...
    stmt = (
        ...
        .where(Favorite.user_id == user_id, Store.is_deleted.is_(False))

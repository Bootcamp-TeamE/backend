...
async def test_unread_count(client: AsyncClient, session: AsyncSession):
    ...
    resp = await client.get("/api/v1/notifications/unread-count", params={"user_id": user.id})
...
async def test_list_unread_filter(client: AsyncClient, session: AsyncSession):
    ...
    listed = (await client.get("/api/v1/notifications", params={"user_id": user.id})).json()
    assert len(listed) == 2
    oldest_id = listed[-1]["id"]
    await client.patch(f"/api/v1/notifications/{oldest_id}/read")

    unread = (await client.get(
        "/api/v1/notifications", params={"user_id": user.id, "unread": True}
    )).json()
    assert len(unread) == 1
...
async def test_mark_all_read(client: AsyncClient, session: AsyncSession):
    ...
    resp = await client.patch("/api/v1/notifications/read-all", params={"user_id": user.id})
    assert resp.status_code == 200
    assert resp.json()["updated"] == 2

    count = (await client.get(
        "/api/v1/notifications/unread-count", params={"user_id": user.id}
    )).json()["count"]
    assert count == 0
